 ...
 * reads it and treats it as claims from an unverified source. Only leads
 * that actually @mention a project/team account are kept (whoever merely
 * posted about it is dropped entirely — see ethos-api.ts's
 * resolveProjectHandlesAndSort), sorted by engagement (likes + retweets).
 *
...
export const tokenSalesTool = tool({
  ...
  execute: async ({ query, chain, saleType }) => {
    ...
    const sales = await resolveProjectHandlesAndSort(rawSales);

    return {
      source: 'x-api-search',
      note:
        sales.length === 0
          ? 'No matching token-sale leads with an identifiable project/team account found in the last 7 days.'
          : `${sales.length} recent public tweets mentioning token sales (last 7 days, search window only — not exhaustive; no structured sale calendar exists), each naming a project/team account, sorted by engagement (likes + retweets, highest first). UNVERIFIED leads — any dates/hard cap/vesting terms are unverified claims from the tweet text, not confirmed facts, and a decent Ethos score is not a safety guarantee.`,
      filtersApplied: { query: query ?? null, chain: chain ?? null, saleType },
      sales,
    };
  },
